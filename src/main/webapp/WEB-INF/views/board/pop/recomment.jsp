    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-sm-12 blog-main">
			...
			<form action="${pageContext.request.contextPath}/board/recommentSubmit" id="recomment-form" class="comment-container" autocomplete="off" name="post/comment_modify" method="post"  enctype='multipart/form-data'><fieldset> 
			<input type="hidden" id = "hidBoardSeq" name ="board_seq" />
			<input type="hidden" id = "hidCommentSeq" name ="comment_seq" />
			<input type="hidden" id = "hidBoardType" name ="board_type" />
			<div class="com_input_wrap clearfix">
			<textarea type="text" name="recomment" id="recomment-box" class="comment-box"
		       ...
		       <div class="com_submit" id="modify-comment-submit"><a href="javascript:recommentSubmit();" 
		       style="position: relative; display: inline-block; margin-right:0.3em; margin-left:10em;"
		        class="btn_com_submit">등록</a>
		       <a href="javascript:recommentCancel();"  class="btn_com_submit"
...
<script>
	$(document).ready(function(){
		...
		$('#hidCommentSeq').val('${param.comment_seq}');
		$('#hidBoardType').val('${param.board_type}');
		$('#hidBoardSeq').val('${param.board_seq}');
		$('#recomment-box').focus();
	});
	
	function recommentSubmit()
	{
		var formId = 'recomment-form';
